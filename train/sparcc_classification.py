...
def _train_sparcc_classification_module(f_train, f_val, sparcc_train, sparcc_val, args, categories=(2, 6, 11)):

    f_i_train, f_ii_train = f_train
    f_i_val, f_ii_val = f_val

    print_frm('Setting up classification dataset')
    train = SPARCCClassificationDataset(f_i_train, f_ii_train, sparcc_train, f_red=args.f_red, categories=categories)
    val = SPARCCClassificationDataset(f_i_val, f_ii_val, sparcc_val, f_red=args.f_red, categories=categories)
...
def _process_fold(args, train, val, f=None, w_i=None, w_di=None):
    ...
    net_s = _train_sparcc_classification_module(f_train, f_val, train[0].sparcc, val[0].sparcc, args)
