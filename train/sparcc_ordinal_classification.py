...
from models.sparcc_cnn import Inflammation_CNN, DeepInflammation_CNN, SPARCC_MLP_OrdinalClassification
...
def _train_sparcc_subclassification_modules(f_train, f_val, sparcc_train, sparcc_val, args):
    ...
    for i in range(len(args.categories)):
        ...
        n_classes = 2
        net = SPARCC_MLP_OrdinalClassification(lr=args.lr, f_dim=args.f_red, f_hidden=args.f_hidden, n_classes=n_classes)
...
def _process_fold(args, train, val, f=None, w_i=None, w_di=None):
    ...
    nets = _train_sparcc_subclassification_modules(f_train, f_val, train[0].sparcc, val[0].sparcc, args)
    ...
    s_true = val[0].sparcc
    y_pred = _predict_sparcc_classification_module(nets, f_val, args)
